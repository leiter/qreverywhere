...
/**
 * Screen for creating QR codes with cryptocurrency wallet addresses
 *
 * Supports:
 * - Bitcoin: bitcoin:{address}?amount={amount}&label={label}&message={message}
...
fun CreateCryptoScreen(
    ...
    Column(
        ...
        // Label Field (Optional, Bitcoin only)
        if (selectedCrypto == CryptoType.BITCOIN) {
            ...
            // Message Field (Optional, Bitcoin only)
            OutlinedTextField(
                value = message,
        ...
        Button(
            onClick = {
                // Validate address
                if (address.isBlank()) {
                    addressError = errorEmptyAddress
                    return@Button
                }
                ...
                try {
                    ...
                    viewModel.saveQrItemFromText(cryptoUri, AcquireType.CREATED)
