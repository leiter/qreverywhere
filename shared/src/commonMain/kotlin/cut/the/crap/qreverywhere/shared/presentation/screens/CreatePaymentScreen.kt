...
/**
 * Screen for creating QR codes with payment request links
 *
 * Supports:
 * - PayPal: https://paypal.me/{username}/{amount}
 * - Venmo: https://venmo.com/{username}?txn=pay&amount={amount}&note={note}
 */
@Composable
fun CreatePaymentScreen(
    ...
    Column(
        ...
        // Note Field (Optional, Venmo only)
        ...
        Button(
            onClick = {
                // Validate username
                if (username.isBlank()) {
                    usernameError = errorEmptyUsername
                    return@Button
                }
                ...
                try {
                    ...
                    viewModel.saveQrItemFromText(paymentUrl, AcquireType.CREATED)
...
private enum class PaymentProvider {
    PAYPAL,
    VENMO
}
